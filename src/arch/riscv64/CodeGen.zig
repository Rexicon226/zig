        ...
        switch (tag) {
            ...
            => {
                ...
                if (lhs_reg.class() == .int) {
                    if (lhs_ty.isAbiInt(zcu)) {
                        const lhs_bits = lhs_ty.intInfo(zcu).bits;
                        if (!math.isPowerOfTwo(lhs_bits)) {
                            try func.truncateRegister(lhs_ty, lhs_reg);
                            try func.truncateRegister(rhs_ty, rhs_reg);
                        }
                    } else {
        ...
        switch (lhs_ty.zigTypeTag(zcu)) {
            ...
            .Int => {
                ...
                if (int_info.bits >= 8 and math.isPowerOfTwo(int_info.bits)) {
                    ...
                    return func.fail("TODO: airAddWithOverflow {d}", .{int_info.bits});
                }
            },
            else => unreachable,
        }
...
fn genSetReg(func: *Func, ty: Type, reg: Register, src_mcv: MCValue) InnerError!void {
    ...
    switch (src_mcv) {
        ...
        .register => |src_reg| {
            ...
            //     const elem_bits = ty.childType(zcu).bitSize(pt);
            //     try func.setVl(.zero, 0, .{
            //         .vsew = bits.VSew.fromBits(elem_bits) orelse
            //             return func.fail("TODO: genSetReg vec -> int bits {d}, make sure to pass in the vector type itself", .{
            //             elem_bits,
            //         }),
...
fn airReduce(func: *Func, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = if (func.liveness.isUnused(inst)) .unreach else result: {
        ...
        try func.setVl(.zero, operand_ty.vectorLen(zcu), .{
            ...
            .vsew = bits.VSew.fromBits(elem_ty.bitSize(pt)) orelse
                return func.fail("TODO: airReduce elem_ty size {}", .{elem_ty.bitSize(pt)}),
            .vma = true,
            .vta = true,
        });
...
fn suggestedVlMul(func: *Func, ty: Type) !bits.VlMul {
    const pt = func.pt;
    assert(ty.isVector(pt.zcu)); // pass in the vector type
