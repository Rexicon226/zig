...
fn simpleMemcpy(dst: [*]u8, src: [*]const u8, size: usize) callconv(.c) usize {
    for (dst[0..size], src[0..size]) |*d, s| {
        d.* = s;
    }
    return size;
}
...
comptime {
    ...
    _ = @import("compiler_rt/addi.zig");

    if (builtin.zig_backend == .stage2_riscv64) {
        @export(&simpleMemcpy, .{ .name = "memcpy", .linkage = .strong });
    } else {
        // Integer routines
